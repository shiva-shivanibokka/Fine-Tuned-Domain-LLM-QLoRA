...
This is a Gradio SDK Space (the only Space type that gets free ZeroGPU). It is
NOT a user-facing UI — the public UI is the separate Next.js app on Vercel,
which calls the `compare` / `generate` API endpoints exposed here via
@gradio/client. The tiny Blocks page below exists only because a Gradio Space
needs one; end users never see it.

Environment:
  ADAPTER_REPO   HF Hub repo holding the trained LoRA/DPO adapter
                 (e.g. "shiva-1993/llama-3.2-3b-cuad-dpo").
...
# Load once at startup on CPU. ZeroGPU attaches a GPU only inside @spaces.GPU
# calls, where we move the model to CUDA.
...
# Minimal Blocks page. Its only job is to register the named API endpoints
# (/compare, /generate) that the Next.js frontend calls via @gradio/client.
with gr.Blocks(title="CUAD Legal LLM — Inference API") as demo:
    gr.Markdown(
        "### CUAD Legal LLM — headless inference API\n"
        "This Space serves the fine-tuned Llama 3.2 3B model. The public UI is "
        "the Next.js app; it calls the `compare` / `generate` API here."
    )
